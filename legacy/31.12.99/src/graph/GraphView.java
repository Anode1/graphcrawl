...
public class GraphView extends Panel implements ArriveEventListener{
    ...
    /**
     * Time in milliseconds how much time not to paint long label
     */
  //  protected final long timeNotToPaint = 3000L;
    ...
    public void moveToCenter(NodeView nodeView){
        ...
        Point v=nodeView.getVelocity(bounds().width/2, bounds().height/2, 30);
        this.setV(v);
...
//System.out.println(nodeView.getNodeModel().getLabel()+":"+nodeView.fromAngle);
    ...
    private NodeView createNeighbour(NodeView parentNodeView, String aChildId, int i, int angleBetween, int edgeLength){
          ...
          aChild.fromAngle=angleBetween*i + parentNodeView.fromAngle-SECTOR_FOR_NEIGHBOURS/2;
    ...
    private void paintEdgesForNode(NodeView nodeView, Graphics g){
      ...
      if(!nodeView.equals(centralNode)){
        //thisNodeAngle=(int)math.rad2Deg(Math.atan2(yDistance,xDistance))+90-angleRange/2;
        thisNodeAngle=(int)math.atan2(yDistance,xDistance)+90-angleRange/2;
      }
      else{
        angleRange=360;
      }
      int fromAngle=thisNodeAngle;//-angleRange/2;
      ...
      for (int i=0; i< numberOfHidden; i++){
        ...
        double dx=math.sin(angle);
        int nonExistingX=nodeView.getMiddleX()+(int)((nodeView.width/2)*dx)+(int)(edgeLength*dx);
