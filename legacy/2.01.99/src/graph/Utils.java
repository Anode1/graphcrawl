...
public void setCentalizedLocationRelativeMe(Component parent, Component me){
    ...
    if(parent==null)return;
     ...
     if(w<parentW)x=parentX+((parentW-w)>>1);
     else x=parentX-((w-parentW)>>1);

     if(h<parentH)y=parentY+((parentH-h)>>1);
     else y=parentY-((h-parentH)>>1);
...
public static Point getCentalizedLocationRelativeParent(Component parent, Component me){
     ...
     if(w<parentW)x=((parentW-w)>>1);
     else x=0;

     if(h<parentH)y=((parentH-h)>>1);
     else y=0;
